...
async def execute(
    ...
    users: List[EventUser] = event_repository.get_users_by_missing_event(EVENT_DEPLOY_CONTRACT.name)
    for user in users:
...
async def _get_deployment_tx_logs(
    ...
    while response and len(response.get("items")):
        ...
        if response.get("next_page_params"):
            response = await explorer_repository.get_transactions_from_account(user.wallet_address, response)
        else:
            response = None
    return None
